...
function checkQuizAnswers() {
  ...
  for (const [question, correct] of Object.entries(correctAnswers)) {
    ...
    if (input.type == "checkbox") {
      const checkedInputs = Array.from(
        document.querySelectorAll(`[name="${question}"]:checked`)
      ).map((input) => input.value);
      if (
        correct.every((value) => checkedInputs.includes(value)) &&
        checkedInputs.length === correct.length
      ) {
  ...
  result_p.innerHTML = `Vous avez ${compt_correct_answers} réponses correctes sur les  ${
    Object.keys(correctAnswers).length
  } questions. (<b>${(
    (compt_correct_answers / Object.keys(correctAnswers).length) *
    100
  ).toFixed(2)}%</b>)`;
...
document
  .getElementById("view_correction")
  .addEventListener("click", () => correctQuizAnswers());
